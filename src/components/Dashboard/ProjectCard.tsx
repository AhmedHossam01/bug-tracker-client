...
const DashboardProjectCard = ({
  project: { name, color, updated_at, id },
}: {
  project: ProjectInterface;
}) => {
  return (
    <Link to={`/dashboard/projects/${id}`}>
      <div className="capitalize h-full group relative shadow-lg dark:shadow-slate-800 bg-slate-100 dark:bg-slate-800 dark:text-white rounded-sm text-slate-700 px-4 py-2 text-4xl flex flex-col justify-between">
        <div
        ...
        <div className="group-hover:text-white z-10 duration-500">
          ...
          <div className="group-hover:text-white duration-500 transition-colors dark:text-gray-400 text-gray-500 text-sm mt-3 flex items-center gap-x-1">
            ...
            Updated {calcTimeAgo(updated_at)} ago
          </div>
        </div>
      </div>
    </Link>
